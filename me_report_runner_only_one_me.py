...
TEST_MODE = False   # When True all emails go ONLY to CC_EMAIL for testing

# !! TO SEND TO SPECIFIC MEs ONLY:
# Set SINGLE_ME_CODES = None            ? send to ALL MEs
# Set SINGLE_ME_CODES = ['310001000001'] ? send to ONE ME only
# Set SINGLE_ME_CODES = ['310001000001','310001000014'] ? send to MULTIPLE MEs
SINGLE_ME_CODES = [
    '310001000001',   # Murshid
    '310001000014',   # Mujaahid
    '310001000010',   # Ajmal
    '310001000021',   # Eman
    # '310001000003',   # Athif
]
...
CC_EMAILS    = [
     "madusha.l@takaful.mv",
      "sarada.jayalath@takaful.mv",
    # Add more CC emails below if needed:
    #  "bi@takaful.mv",
]
...
def get_report_period():
    ...
    cursor.execute("""
        SELECT
            manual_month,
            manual_year,
            IF(manual_month = 0, MONTH(CURDATE()), manual_month),
            IF(manual_year  = 0, YEAR(CURDATE()),  manual_year)
...
def fetch_me_list(rpt_month, rpt_year):
    ...
    if SINGLE_ME_CODES:
        # Send to SPECIFIC MEs only (one or multiple)
        placeholders = ",".join(["%s"] * len(SINGLE_ME_CODES))
        cursor.execute(f"""
            SELECT me_target_code AS me_code, me_name, email
            FROM bidataforme.me_master
            WHERE me_target_code IN ({placeholders})
              AND email IS NOT NULL AND email != ""
              AND (is_active IS NULL OR is_active = 1)
            ORDER BY me_name
        """, tuple(SINGLE_ME_CODES))
    else:
...
def send_me_email(me_name, me_email, html_body, subject):
    # Ensure all email addresses are clean strings
    me_email_clean = str(me_email).strip()
    cc_email_clean = [str(e).strip() for e in CC_EMAILS]

    if TEST_MODE:
        to_addr    = cc_email_clean[0]
        recipients = cc_email_clean
        test_tag   = f"[TEST - {me_name}] "
    else:
        to_addr    = me_email_clean
        recipients = [me_email_clean] + cc_email_clean
        test_tag   = ""
    ...
    try:
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as server:
            ...
            server.sendmail(str(SMTP_USER), recipients, msg.as_string())
        if TEST_MODE:
            print(f"[Email TEST] Sent {me_name} report to {", ".join(cc_email_clean)}")
        else:
            print(f"[Email] Sent to {me_name} <{me_email_clean}>")
...
if __name__ == "__main__":
    ...
    if SINGLE_ME_CODES:
        print(f"[Mode]  Single ME: {SINGLE_ME_CODES}")
    if TEST_MODE:
        print(f"[Mode]  TEST MODE - emails go to {", ".join(CC_EMAILS)}")
    print(f"{'='*60}\n")
    ...
    for me in me_list:
        me_code  = str(me["me_code"]).strip()
        me_name  = str(me["me_name"]).strip()
        me_email = str(me["email"]).strip()
